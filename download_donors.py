...
# и раньше.
# v19 (по просьбе пользователя, после серии подтверждений v14/v17/v18):
# удалённая фильтрация к этому моменту подтверждённо проваливается на
# подавляющем большинстве хромосом (1-11, 14-18) на реальных сетях —
# остаётся лишь единичный смысл пробовать её для необследованных 12,
# 13, 19-22, но экономия (секунды/минуты на маленьких хромосомах) не
# перевешивает риск повторить ту же историю с зависаниями на
# REMOTE_CHROM_TIMEOUT. Пропускаем remote-путь для ВСЕХ хромосом —
# сразу используется полное скачивание (process_chromosome(), уже
# ускоренное через aria2c/curl/докачку — см. 1.2/1.4), без исключений.
# process_chromosome_remote()/diagnose_remote_filter() (кнопка
# "Диагностика сети") остаются в коде нетронутыми — просто не
# вызываются по умолчанию на основном пути скачивания доноров. Чтобы
# вернуть прежнее выборочное поведение — передайте свой набор через
