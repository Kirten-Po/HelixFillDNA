...
#: лифтовером. Опущен с 10.0 до 8.0 по замеру на реальных прогонах:
#: файл FTDNA -> шаблон v5 имел редкий хвост 9,89 % (то есть попадал под
#: старый порог в HRC) и дал при этом ХУДШИЙ nocall из всех — 11,97 %.
#: Между 4 % и 8 % наблюдений нет ни одного, поэтому ниже 8.0 опускать
#: нечего: это была бы непроверенная зона. Прежняя посылка "ошибиться в
#: сторону HRC дешевле" замером опровергнута — см. TOPMED_TEMPLATES.
RARE_TAIL_PCT_THRESHOLD = 8.0
...
#: Шаблоны вывода, позиции которых HRC покрывает плохо, — решающий
#: сигнал, более сильный чем состав входного чипа. Замер на собранных
#: файлах: шаблон v3 (952 034 позиции, контент 23andMe 2010 года) даёт
#: 1,7-2,4 % nocall, а genotek и v5 (оба на Illumina GSA, с экзонным и
#: редким контентом) — 6,1-12,0 % на тех же исходниках. Проверка на
#: chr22: из 885 позиций, оставшихся nocall на HRC, 732 (82,7 %) есть в
#: TopMed, то есть это не экзотика чипа, а обрезка HRC по MAC>=5.
TOPMED_TEMPLATES = frozenset({"genotek", "v5"})
...
def recommend_panel(
    ...
    *,
    ...
    """
    Читаемое правило: шаблон вывода из TOPMED_TEMPLATES ИЛИ неевропейское
    происхождение ИЛИ тяжёлый редкий хвост -> TopMed; иначе HRC как более
    быстрый и без лифтовера.

    template_kind — ключ формата итогового файла ("v3" | "v5" |
    "genotek"). Это САМЫЙ СИЛЬНЫЙ сигнал (см. TOPMED_TEMPLATES): nocall
    возникает на позициях ШАБЛОНА, которых нет в панели, поэтому состав
    входного чипа предсказывает его хуже, чем состав выхода.
    template_kind=None означает "формат не известен" — правило тогда
    работает по-старому, только на составе чипа.
    ...
    gsa_template = template_kind in TOPMED_TEMPLATES

    if gsa_template:
        reasons.append(
            f"шаблон вывода «{template_kind}» построен на Illumina GSA — "
            f"его экзонный и редкий контент HRC не покрывает (обрезка "
            f"MAC>=5). Замер на chr22: 82,7 % позиций, оставшихся nocall "
            f"на HRC, в TopMed есть"
        )
