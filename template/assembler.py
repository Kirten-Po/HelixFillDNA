...
def load_imputed_genotypes(
    ...
    отсекается) при gp_threshold=0.90 ОДНИМ числом не выражается: он
    зависит от того, насколько осмыслен Rsq в конкретном прогоне.

    Решающий фактор — ЧИСЛО ДОНОРСКИХ ОБРАЗЦОВ. Rsq считается по
    разбросу дозировок среди образцов задания; при 20 донорах их 21, и
    Rsq схлопывается к нулю на всём, кроме самых частых вариантов, —
    тот же эффект, что у DR2 на пяти образцах в семейном пайплайне.
    Замер на ОДНОМ образце и одной панели, меняется только число
    доноров (Атлас -> genotek, HRC, chr21+22):

        20 доноров:   Rsq 96,50 %  ->  GP 97,80 %   (+1,31 п.п.)
        503 донора:   Rsq 97,90 %  ->  GP 97,84 %   (-0,07 п.п.)

    Второй фактор — какую долю трафарета чип кроет НАПРЯМУЮ: чем меньше,
    тем больше позиций зависит от импутации и тем сильнее метрика. Замер
    на FTDNA-чипе, 20 доноров, HRC, chr1/11/19/21/22:

        трафарет v3       92,67 %  ->  95,69 %   (+3,03 п.п.)
        трафарет genotek  79,65 %  ->  91,55 %  (+11,91 п.п.)
        трафарет v5       75,78 %  ->  91,02 %  (+15,24 п.п.)

    Практический вывод: при 20 донорах (умолчание обычного режима) GP
    даёт от +1 до +15 п.п.; при полном наборе доноров — около нуля, там
    Rsq и так работает. GP позволяет остаться на дешёвых 20 донорах и
    получить заполняемость, которая раньше требовала всех 503.

    ⚠ Ранее здесь стояли числа "genotek +2,15 / v5 +1,76 / v3 -0,09".
    Они неверны дважды: мерились на прогоне с 503 донорами (то есть не
    на умолчании), а для v5 и v3 — вдобавок на позициях, отфильтрованных
    списком из трафарета genotek, то есть на смещённой подвыборке в 20 %
    строк. Утверждение "на v3 выигрыша нет" было прямо ошибочным.
