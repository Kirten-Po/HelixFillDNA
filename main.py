...
#: Сборка генома, в которой живут трафареты, скелет (template/skeleton.py)
#: и координаты чипа на входе пайплайна. Всё, что приходит из панели в
#: другой сборке, обязано быть перенесено сюда обратно перед сборкой
#: итогового файла.
SKELETON_BUILD = "grch37"


def _panel_needs_liftover(panel: Optional[str]) -> bool:
    """
    Требует ли панель переноса координат относительно скелета трафарета.

    ⚠ Почему это отдельная функция, а не сравнение с DEFAULT_PANEL.
    Раньше в пяти местах стояло `panel != DEFAULT_PANEL` — и читалось это
    как "панель не HRC, значит нужен лифтовер", потому что DEFAULT_PANEL
    был "hrc". В 1.3.2 DEFAULT_PANEL стал "topmed", и все пять условий
    молча инвертировались: для TopMed лифтовер перестал выполняться, а
    для HRC начал запрашиваться там, где не нужен.

    Цена была катастрофической и тихой. На Этапе 7 пропуск лифтовера
    означал, что скелет остаётся в GRCh37, а дозы и измерения — в
    GRCh38: позиции одного локуса в разных сборках почти никогда не
    совпадают числом, поэтому в итоговый файл попадали только случайные
    совпадения. Реальный прогон ATLAS-TOPMED: заполнено 1,15 % вместо
    ожидаемых ~98 %, при том что словарь генотипов был собран правильно
    (573 534 записи) и ни одной ошибки в лог не попало.

    Признак панели, а не имя, потому что вопрос именно про сборку: если
    завтра появится третья панель в GRCh38 или панель в GRCh37, условие
    останется верным без правок.
    """
    return _panel_config(panel)["genome_build"] != SKELETON_BUILD
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        help=("Чем отсекать ненадёжные импутированные вызовы. gp (по "
              ...
              "уверена на 99 %%. Размер выигрыша зависит от числа доноров "
              "(при 20 Rsq почти не работает, при 503 работает) и от того, "
              "какую долю трафарета чип кроет напрямую: замеры дают от "
              "+1 п.п. до +15 п.п. заполняемости, на полном наборе доноров "
              "— около нуля."))
    parser.add_argument(
...
def main() -> None:
    ...
    if _panel_needs_liftover(args.panel) and not _supports_liftover(args.source):
        logger.warning(
