...
Изменения в этой версии (по промту "Комплексное улучшение GUI"):
 ...
 - Шаг 1 промта "Поддержка выбора референсной панели HRC / TopMed r3":
   ...
   ⚠ Панель по умолчанию с тех пор сменилась: в 1.3.2 DEFAULT_PANEL
   переведён с 'hrc' на 'topmed'. Запись выше ("по умолчанию HRC")
   описывает состояние на момент того шага, а не сегодняшнее.
"""
...
#: (метрика из GWAS). Замеры на реальных прогонах: переход даёт от +1 до
#: +15 п.п. заполняемости при 20 донорах (умолчание обычного режима) и
#: около нуля при полном наборе, где Rsq и так осмыслен,
...
class App(ctk.CTk):
    ...
    def _build_settings_tab(self):
        ...
        ctk.CTkLabel(
            ...
            text=("GP — вероятность того, что генотип У ЭТОГО человека В ЭТОЙ "
                  ...
                  "99 %.\n"
                  "Выигрыш тем больше, чем меньше донорских образцов: при "
                  "20 (умолчание) Rsq почти\nне работает и GP даёт от +1 до "
                  "+15 п.п. заполняемости; при всех 503 — около нуля."),
            justify="left", text_color="gray60", wraplength=700,
        ...
        # Согласовать состояние полей с выбранной метрикой СРАЗУ, а не
        # только при первом переключении: иначе на старте выбран GP, но
        # поле Rsq остаётся активным, и непонятно, какое из двух работает.
        # Вызывается здесь, потому что оба поля уже созданы выше.
    ...
    def _apply_simple_presets(self):
        ...
        # Референсная панель в обычном режиме — всегда DEFAULT_PANEL,
        # то есть с 1.3.2 TopMed (раньше здесь был HRC, и комментарий
        # обещал GRCh37 без лифтовера). Причина смены: при неизвестном
        # составе чипа падать надо на панель, которая покрывает больше —
        # nocall возникает на позициях ШАБЛОНА ВЫВОДА, а трафареты
        # genotek и v5 построены на Illumina GSA, который HRC с её
        # обрезкой по MAC>=5 не покрывает. Цена — лифтовер GRCh37 <-> GRCh38
        # и свой комплект референса/доноров на каждую панель.
        # Значение читается из константы, а не дублируется здесь.
